<?php declare(strict_types=1);

namespace App\Identity\Presentation\Password\Reset;

use App\Shared\Presentation\Responder;
use App\Shared\Presentation\Response\ApiResponse;
use App\Identity\Application\Password\Reset\Exception\InvalidResetEmailException;
use App\Identity\Application\Password\Reset\Exception\InvalidResetTokenException;
use App\Identity\Application\Password\Reset\Exception\PasswordResetFailedException;
use App\Shared\Application\Result\Result;
use Illuminate\Http\Response;

/**
 * @phpstan-extends Responder<null, \Throwable>
 */
final class ResetPasswordResponder extends Responder
{
    /**
     * @phpstan-param Result<null, \Throwable> $result
     * @phpstan-return ApiResponse
     */
    public function respond(Result $result): ApiResponse
    {
        return $result->match(
            onSuccess: fn () => new ApiResponse(
                data: ['message' => __('identity.password.reset.success')],
                status: Response::HTTP_OK
            ),
            onError: fn (\Throwable $e) => match (true) {
                $e instanceof \DomainException,
                $e instanceof InvalidResetEmailException,
                $e instanceof InvalidResetTokenException => new ApiResponse(
                    data: ['message' => $e->getMessage()],
                    status: Response::HTTP_BAD_REQUEST
                ),
                $e instanceof PasswordResetFailedException => new ApiResponse(
                    data: ['message' => __('identity.password.reset.failed')],
                    status: Response::HTTP_INTERNAL_SERVER_ERROR
                ),
                default => new ApiResponse(
                    data: ['message' => __('identity.password.reset.unknown')],
                    status: Response::HTTP_INTERNAL_SERVER_ERROR
                ),
            }
        );
    }
}
